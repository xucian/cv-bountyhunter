...
export class RealGitHubService implements IGitHubService {
  ...
  async createSolutionPR(
    ...
    const uniqueId = Date.now().toString(36).slice(-6); // Short unique suffix
    const branchName = `codebounty/fix-issue-${issue.number}-${uniqueId}`;
    ...
    try {
      ...
      // Fetch latest and create unique branch from origin/main
      await execAsync('git fetch origin');

      try {
        await execAsync(`git checkout -b ${branchName} origin/main`);
      } catch {
        // Fallback: checkout main first, then create branch
...
`;
      ...
      const timeFormatted = solution.timeMs < 1000
        ? `${solution.timeMs}ms`
...
${solution.code.length > 500 ? '> See changed files for implementation details.' : '```\n' + solution.code + '\n```'}

---
<sub>Generated by **${agentName}** in ${timeFormatted} via [Bounty Hunter](https://github.com/xucian/cv-bountyhunter)</sub>`;
      } else {
        prBody = `Closes #${issue.number}

## What this PR does
AI-generated solution for: **${issue.title}**

## Implementation
See [\`${solutionFile}\`](${solutionFile}) for the complete solution.
...
---
<sub>Generated by [Bounty Hunter](https://github.com/xucian/cv-bountyhunter) - AI agents competing for bounties with crypto payments</sub>`;
      }
